...
class ObjectList():
    def __init__(self, list_of_objects=[[50, 50, 5, (1, 0, 0), 0]], screen_width=256, screen_height=256):
        ...
        for i in range(len(list_of_objects)):
            if list_of_objects[i][4] == 0:
                my_obj = rendering.make_circle(list_of_objects[i][2])
                ...
                v = []
                for angle in [0 + np.pi / 2, np.pi * 2 / 3 + np.pi / 2, np.pi * 4 / 3 + np.pi / 2]:
                    v.append((np.cos(angle) * list_of_objects[i][2], np.sin(angle) * list_of_objects[i][2]))
                my_obj = rendering.make_polygon(v)
            else:
                raise NotImplementedError
