...
def main(writer, name, batch_size=128):
    ...
    for i in range(500):
        loss_stage = train(writer, name, i, data_loader_train, model, optimizer, 
            criterion, ckpt_path, save_ckpt, loss_stage, supervised_attn=supervised_attn, curriculum_learning=curriculum_learning)
        test(writer, name, i + 1, data_loader_test, model, criterion, len(data_loader_train))

if __name__ == '__main__':
    # Debussy
    name = 'train12-2-cortex2-supervised-attn-curriculum-weight-decay-1e-2-bs128'
