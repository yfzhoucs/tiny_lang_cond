...
class ControllerBN(nn.Module):
    def __init__(self, embedding_size, num_joints):
        super(ControllerBN, self).__init__()
        self.layer1 = nn.Linear(embedding_size, 16 * 16 * 4)
        self.layer2 = nn.Linear(16 * 16 * 4, 16 * 16 * 2)
        self.layer3 = nn.Linear(16 * 16 * 2, 16 * 16)
        self.layer4 = nn.Linear(16 * 16, num_joints)

        self.layer1_bn = nn.BatchNorm1d(16 * 16 * 4)
        self.layer2_bn = nn.BatchNorm1d(16 * 16 * 2)
        self.layer3_bn = nn.BatchNorm1d(16 * 16 * 1)

    def forward(self, x):
        x = F.selu(self.layer1_bn(self.layer1(x)))
        x = F.selu(self.layer2_bn(self.layer2(x)))
        x = F.selu(self.layer3_bn(self.layer3(x)))
        x = self.layer4(x)
        return x


class DMP(nn.Module):
    def __init__(self, nfunctions=11, scale=0.012, device=torch.device('cuda')):
        super(DMP, self).__init__()
        self.nfunctions = nfunctions
        self.scale = scale
        self.centers = torch.tensor(np.linspace(0.0, 1.01, self.nfunctions, dtype=np.float32)).to(device)

    def _evaluate_basis_functions(self, phis):
        # phis: (batch_size, num_joints, num_phi_to_evaluate)
        # self.centers: (11,)

        # Expand centers to be (batch_size, num_joints, num_phi_to_evaluate, 11)
        # Expand phis to be (batch_size, num_joints, num_phi_to_evaluate, 11)
        centers = self.centers.unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(*phis.shape, 1)
        phis = phis.unsqueeze(3).repeat(1, 1, 1, 11)

        # Evaluate the normal distribution pdf
        # x is phis
        # mu is centers
        # sigma^2 is self.scale
        pos_weights = torch.exp(- (torch.square(phis - centers) / (2.0 * self.scale)))
        return pos_weights

    def _calculate_trajectory_points(self, pos_weights, dmp_weights):
        # pos_weights: (batch_size, num_joints, num_phi_to_evaluate, 11)
        # dmp_weights: (batch_size, num_joints, 11)

        # Expand dmp_weights to (batch_size, num_joints, num_phi_to_evaluate, 11)
        dmp_weights = dmp_weights.unsqueeze(2).repeat(1, 1, pos_weights.shape[2], 1)

        # Weighted sum across dmp weights
        # Return trajectory_points: (batch_size, num_joints, num_phi_to_evaluate)
        weighted_dmp_weights = torch.mul(pos_weights, dmp_weights)
        trajectory_points = torch.sum(weighted_dmp_weights, dim=3, keepdim=False)
        return trajectory_points


    def forward(self, dmp_weights, phis):
        # dmp_weights refers to the weights that help re-create the trajectory
        # dmp_weights: (batch_size, num_joints, 11)
        # phi refers to the phase position, between 0 and 1
        # phis: (batch_size, num_joints, num_phi_to_evaluate)

        # Calculate the positional weights according to the phi
        # return pos_weights: (batch_size, num_joints, num_phi_to_evaluate, 11)
        pos_weights = self._evaluate_basis_functions(phis)

        # Calculate the weighted sum of positional weights and trajectory weights
        # return results: (batch_size, num_joints, num_phi_to_evaluate)
        results = self._calculate_trajectory_points(pos_weights, dmp_weights)
...
# Some code from DETR
class Backbone_Perception(nn.Module):
    def __init__(self, img_size, num_joints, num_tasks, embedding_size, add_displacement=False, ngf=64, channel_multiplier=4, device=torch.device('cuda'), ignore_default_2_joint_head=False):
        super(Backbone_Perception, self).__init__()
        ...
        self.joint_encoder_3_joints = JointEncoder(2 * 2, embedding_size)
        ...
        # Cortex Module
        self.seg_embed1 = nn.Embedding(7, embedding_size)
        self.seg_embed2 = nn.Embedding(7, embedding_size)
        self.seg_embed3 = nn.Embedding(7, embedding_size)
        self.seg_embed4 = nn.Embedding(7, embedding_size)
        ...
        self.controller_3_joints = ControllerBN(embedding_size, num_joints * 2)
    ...
    def _get_segment_embed_(self, batch_size, layer):
    ...
    def _get_position_embed_(self, batch_size):
        position_ids = np.arange(262)
        position_ids = torch.tensor(position_ids, dtype=torch.int32).unsqueeze(0).repeat(batch_size, 1).to(self.device)
    ...
    def forward(self, img, joints, target_id, attn_mask=None):
        ...
        if not self.ignore_default_2_joint_head:
            ...
            action_pred = self.controller_3_joints(state_embedding4[:,1,:])
            joints_pred = self.joints_predictor_3_joints(state_embedding3[:,3,:])
...
class Backbone(nn.Module):
    def __init__(self, img_size, num_joints, num_tasks, embedding_size, add_displacement=False, ngf=64, channel_multiplier=4, device=torch.device('cuda')):
        ...
        self.backbone_perception = Backbone_Perception(img_size, num_joints*11, num_tasks, embedding_size, add_displacement=add_displacement, ngf=64, channel_multiplier=4, device=device, ignore_default_2_joint_head=True)
        self.dmp = DMP(device=device)
        self.device = device

    def forward(self, img, joints, target_id, phis, attn_mask=None):
        # dmp_weights refers to the weights that help re-create the trajectory
        # dmp_weights: (batch_size, num_joints, 11)
        # phi refers to the phase position, between 0 and 1
        # phis: (batch_size, num_joints, num_phi_to_evaluate)
        # return: (batch_size, num_joints, num_phi_to_evaluate)

        # Perception; Reshape dmp_weights
        batch_size = img.shape[0]
        dmp_weights, target_position_pred, displacement_pred, attn_map, attn_map2, attn_map3, attn_map4, joints_pred = self.backbone_perception(img, joints, target_id, attn_mask)
        dmp_weights = dmp_weights.reshape(batch_size, 2*self.num_joints, 11)
        
        # # Create phi to evaluate. num_samples_in_trajectory phis for each joint.
        # phis = torch.tensor(np.linspace(0.0, 1.01, num_samples_in_trajectory, dtype=np.float32)).unsqueeze(0).unsqueeze(0).repeat(batch_size, 2*self.num_joints, 1).to(self.device)

        # Calculate the full trajectory
        trajectory = self.dmp(dmp_weights, phis)

        return trajectory, target_position_pred, displacement_pred, attn_map, attn_map2, attn_map3, attn_map4, joints_pred
